    ...
    if(init)
    {
      ...
      for(i = 0, k = 0; i < points[1].size(); i++)
			...
			if(diffX.rows > 10)
			{
				...
				compX = (attRoll - attRollPrev)*(NO_OF_COLUMNS/2)/(FOV);
				compY = (attPitch - attPitchPrev)*(NO_OF_ROWS/2)/(FOV);

				ofDispX = medianX + compX;
				ofDispY = medianY + compY;	

        dispX = (2 * altitude * tan(FOV/2) * ofDispX) / (NO_OF_COLUMNS / 2);
        dispY = (2 * altitude * tan(FOV/2) * ofDispY) / (NO_OF_ROWS / 2);

        currPos.x = prevPos.x + roundf(dispX * 100) / 100;
        currPos.y = prevPos.y + roundf(dispY * 100) / 100;

        visionPose.pose.position.x = roundf(currPos.y * 100) / 100;
        visionPose.pose.position.y = roundf(currPos.x * 100) / 100;
